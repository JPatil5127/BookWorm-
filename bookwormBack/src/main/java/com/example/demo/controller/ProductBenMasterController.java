...
@RestController
@CrossOrigin
@RequestMapping("/api/productBeneficiaries")
public class ProductBenMasterController {
    ...
    @GetMapping("/ben/{productId}")
    public ResponseEntity<ProductBenMaster> getProductBenefitsByProductId(@PathVariable int productId) {
        ProductBenMaster productBenefits = productBenMasterService.findByProductId(productId);
        
        if (productBenefits==null) {
            return ResponseEntity.notFound().build();
        }
        
        return ResponseEntity.ok(productBenefits);
    }
